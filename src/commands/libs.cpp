...
void get_latest_tag( char* out, const char* libname )
{
  ...
  char* owner = nullptr;
  char* repo = nullptr;
  char* libname_copy = strdup(libname); // Duplicate the input string for manipulation
  ...
  *pos = '\0'; // Split the string
  ...
  if ( curl )
  {
    ...
    snprintf(url, sizeof(url), "https://api.github.com/repos/%s/%s/tags", owner, repo);
  ...
  {
    std::cerr << "Failed to parse JSON: " << rapidjson::GetParseError_En(document.GetParseError()) << std::endl;
    free(readBuffer);
    free(libname_copy);
...
int install_to_global_libs( char* lib )
{
  return 0;
}

int install_to_global_libs( char* lib, char* version )
{
  return 0;
}

int remove_from_global_libs( char* lib )
{
  return 0;
}

int remove_from_global_libs( char* lib, char* version )
{
  return 0;
}