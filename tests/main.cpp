...
TEST_CASE("conf.hpp/add_lib_to_toml", "[int add_lib_to_toml(const char*, char*)]")
{
  REQUIRE(add_lib_to_toml(strdup("catchorg_Clara"), strdup("v1.1.5")) == 0);
}

TEST_CASE("conf.hpp/take_lib_from_toml", "[int take_lib_from_toml(const char*)]")
{
...
TEST_CASE("files.hpp/generate_new_content", "[int generate_new_content(const char*)]")
{
...
TEST_CASE("libs.hpp/install_libraries", "[int install_libraries(toml::parse_result&)]")
{
  toml::parse_result toml_file = toml::parse_file("clarbe.toml");
  // Later
}

TEST_CASE("libs.hpp/get_latest_tag", "[int get_latest_tag(char*, const char*)]")
{
  char* output = (char*)malloc(1);
...
TEST_CASE("libs.hpp/write_callback", "[int write_callback(void*, size_t, size_t, char**)]")
{
  // Later
}
*/
